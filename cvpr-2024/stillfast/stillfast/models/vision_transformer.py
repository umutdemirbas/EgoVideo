...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, return_f3d=False):
    ...
    def forward(self, x, return_f3d=False):
        if return_f3d:
            return self.forward_features(x, return_f3d)
...
@register_model
def vit_base_patch16_224_umt_k400(pretrained=True, ckpt_path="/mnt/petrelfs/chenguo/pretrained_models/umt/b16_ptk710_ftk710_ftk400_f8_res224.pth", **kwargs):
    ...
    if pretrained:
        ...
        if 'state_dict' in state_dict:
            ...
            ms, ue = model.load_state_dict(new_state_dict, strict=False)
        else:
            ms, ue = model.load_state_dict(state_dict, strict=False)
        print(f"Load pretrained ckpt from {pretrain_ckpt}")
        print(f"Missing {ms}", flush=True)
        print(f"Unexpected {ue}", flush=True)
    else:
...
@register_model
def vit_base_patch16_384(pretrained=False, ckpt_path="", **kwargs):
...
@register_model
def vit_large_patch16_224(pretrained=False, ckpt_path="", **kwargs):
    model = VisionTransformer(patch_size=16,
                              embed_dim=1024,
                              depth=24,
                              num_heads=16,
